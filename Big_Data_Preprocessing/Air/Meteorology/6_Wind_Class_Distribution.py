...
def main():
    ...
    for index in range(rows.size):
        ...
        for index in range(Wind_Direction.size):
            ...
            df.loc[index] = value
        hist, bin_edges = np.histogram(Wind_Speed, bins=np.array([0, 0.5, 2.10, 3.60, 5.70, 8.80, 11.10]))
        ...
        plt.bar(labels, percentages, color='blue', align='center', width=0.8)
        plt.xlabel('Wind Speed Range (m/s)')
        plt.ylabel('Percentage (%)')
        plt.title('Wind Speed Class Frequency Distribution')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig(f'G:/Big_Data_Preprocessing/Air/Meteorology/Result/Output/Wind_Class_{row}_{col}.png')
        plt.close('all')
