...
for feature in layer:
    ...
    # print(attributes['lat'], attributes['lon'])
...
path_tiff_files = "D:/Code_Result/Result/NASA_TUTORIAL_SPACE/EMISSION6/ANT/TIFF/"
...
output_folder_Ant = "D:/Code_Result/Result/NASA_TUTORIAL_SPACE/EMISSION6/ANT/EXCEL/RV/"
...
ds_Ant = xr.open_dataset(data_Ant_co)
df_Ant = pd.DataFrame()


for substance in substance_Ant:
    ds_Ant = xr.open_dataset(globals()[f"data_Ant_{substance}"])
    for variable in variable_Ant:
        if variable in ds_Ant.variables:
            for time_df in time_Data_excel_2023:
                ...
                df_Ant["LON"], df_Ant["LAT"] = df_Excel["POINT_X"], df_Excel["POINT_Y"]
...
for raster in tqdm(rasters, desc="Processing files", total=total_iterations):
    ...
    while (parts[i] != "Ant"):
        ...
        i += 1
    occupation = parts[i]
    # value_substance = list_Value_Substance[substance]
    for value_substance in list_Value_Substance[substance]:
        ...
        excel_file_path_df = f"D:/Code_Result/Result/NASA_TUTORIAL_SPACE/EMISSION6/ANT/EXCEL/DF/{occupation}_{value_substance_x}_{sector_x}_{int(date[0:2])}_{substance}.xlsx"
        excel_file_path_cb = f"D:/Code_Result/Result/NASA_TUTORIAL_SPACE/EMISSION6/ANT/EXCEL/CB/{occupation}_{value_substance_x}_{sector_x}_{int(date[0:2])}_{substance}.xlsx"
        # excel_file_path_df = f"G:/NASA_TUTORIAL_SPACE/TEMP/Emission_nam_2023/CAMS_GLOB_ANT_EXCEL/DF_Test/{occupation}_{value_substance_x}_{sector_x}_{int(date[0:2])}_{substance}.xlsx"
        # excel_file_path_cb = f"G:/NASA_TUTORIAL_SPACE/TEMP/Emission_nam_2023/CAMS_GLOB_ANT_EXCEL/CB_Test/{occupation}_{value_substance_x}_{sector_x}_{int(date[0:2])}_{substance}.xlsx"
