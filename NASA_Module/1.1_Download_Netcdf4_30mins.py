...
for month in range(1, 2, 1):
    for day in range(1, 14, 1):
        ...
        save_dir = f"D:/Code_Result/Result/NASA/NASA_Data_30minutes/Y23M{month_string}/D{day_string}"
        os.makedirs(save_dir, exist_ok=True)
        response = requests.get(url)
        soup = BeautifulSoup(response.content, "html.parser")
        links = soup.find_all("a")
        filtered_links = [link.get("href") for link in links if link.get("href").startswith("GEOS-CF.v01.rpl.aqc_tavg_1hr_g1440x721")]
